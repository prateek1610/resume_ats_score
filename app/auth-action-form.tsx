...
export function UpdatePasswordForm({ returnTo }: { returnTo: string }) {
  ...
  return (
    <form className="auth-fields" action="/auth/password-update" method="post" onSubmit={submit}>
      <input type="hidden" name="returnTo" value={returnTo} />
      <div className="auth-field-group"><label htmlFor="new-password"><span>New password</span></label><span className="password-field"><input id="new-password" name="password" type={showPassword ? "text" : "password"} autoComplete="new-password" minLength={15} maxLength={128} required placeholder="At least 15 characters" /><button type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword((value) => !value)}>{showPassword ? "Hide" : "Show"}</button></span></div>
