...
import { isReportView } from "@/lib/report-view";
...
export default async function ReportViewPage({ params }: { params: Promise<{ id: string; view: string }> }) {
  const { id, view } = await params;
  if (!isReportView(view)) notFound();

  const user = await requireAppUser(`/reports/${id}/${view}`);
  ...
  if (view === "job-match" && report.mode !== "job_match") notFound();
  ...
  return (
    <main className="app-shell">
      ...
      <div className="report-wrap report-workspace-wrap">
        <div className="report-toolbar report-toolbar-compact">
        ...
        <AnalysisView analysis={analysis} view={view} basePath={`/reports/${report.id}`} structuredResume={report.structuredResume} />
